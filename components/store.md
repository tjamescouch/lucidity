...
- syncs the tree to external persistent storage on each curation pass and on exit
- loads the tree from persistent storage on boot (if local copy is missing)
...
- on boot, if both local and remote exist, the more recent one wins
